...
class vec { float x=0,y=0; 
 ...
 // MODIFY
  ...
  vec turn() {float m=x; x=-y; y=m; return this;};
...
void show(pt P, float s, vec V) {show(P,W(s,V));}                                                     // show sV as line-segment from P 
void arrow(pt P, float s, vec V) {arrow(P,W(s,V));}                                                   // show sV as arrow from P 
...
void arrow(pt P, vec V) {show(P,V);  float n=n(V); if(n<0.01) return; float s=max(min(0.2,20./n),6./n);       // show V as arrow from P 
     pt Q=P(P,V); vec U = W(-s,V); vec W = R(W(.3,U)); beginShape(); v(P(P(Q,U),W)); v(Q); v(P(P(Q,U),-1,W)); endShape(CLOSE);}; 

// weighted sum 
vec W(float s,vec V) {return V(s*V.x,s*V.y);}                                                      // sV
vec W(vec U, vec V) {return V(U.x+V.x,U.y+V.y);}                                                   // U+V 
vec W(vec U,float s,vec V) {return W(U,W(s,V));}                                                   // U+sV
vec W(float u, vec U, float v, vec V) {return W(W(u,U),W(v,V));}                                   // uU+vV ( Linear combination)
...
vec S(vec U, vec V, float s) {float a = angle(U,V); float u = n(U); float v=n(V); return W(pow(v/u,s),R(U,s*a)); } // steady interpolation from U to V

// measure 
float dot(vec U, vec V) {return U.x*V.x+U.y*V.y; }                                                     // dot(U,V): U*V (dot product U*V)
float det(vec U, vec V) {return dot(U,R(V)); }                                                         // det | U V | = scalar cross UxV 
...
// Parabola
pt Parabola(pt A, pt B, pt C, float t) {pt P=L(A,B,t*2); pt Q=L(B,C,t*2-1); return  L(P,Q,t);}

// Bezier 
pt B(pt A, pt B, pt C, float t) {pt P=L(A,B,t); pt Q=L(B,C,t); return L(P,Q,t);}                      // point on quadratic Bezier
pt B(pt A, pt B, pt C, pt D, float t) {pt P=B(A,B,C,t); pt Q=B(B,C,D,t); return L(P,Q,t);}            // point on cubic Bezier
pt B(pt A, pt B, pt C, pt D, pt E, float t) {pt P=B(A,B,C,D,t); pt Q=B(B,C,D,E,t); return L(P,Q,t);}  // point on quartic Bezier
pt B(pt A, pt B, pt C, pt D, pt E, pt F, float t) {pt P=B(A,B,C,D,E,t); pt Q=B(B,C,D,E,F,t); return L(P,Q,t);} // point on quintic Bezier
...
pt I(float a, pt A, float b, pt B, float c, pt C, float d, pt D, float e, pt E,float f, pt F, float t) {pt P=I(a,A,b,B,c,C,d,D,e,E,t); pt Q=I(b,B,c,C,d,D,e,E,f,F,t); return I(a,P,f,Q,t);}

// Interpolation uniform
pt I(pt A, pt B, pt C, float t) {return I(0,A,1,B,2,C,t*2);}                               // P(0)=A, P(1)=B
pt I(pt A, pt B, pt C, pt D, float t) {return I(0,A,1,B,2,C,3,D,t*3);}                     // P(0)=A, P(1/2)=B, P(1)=C
pt I(pt A, pt B, pt C, pt D, pt E, float t) {return I(0,A,1,B,2,C,3,D,4,E,t*4);}            // P(0)=A, P(1/3)=B, P(2/3)=C, P(1)=D
pt I(pt A, pt B, pt C, pt D, pt E, pt F, float t) {return I(0,A,1,B,2,C,3,D,4,E,5,F,t*5);}
...
pt D(pt A, pt B, pt C, pt D, pt E, pt F, float t) {float a=0, b=d(A,B), c=b+d(B,C), d=c+d(C,D), e=d+d(D,E), f=e+d(E,F); return I(a,A,b,B,c,C,d,D,e,E,f,F,a+t*(f-a));}

// NUBS 
pt S(float a, pt A, float b, pt B, float c, pt C, float d, pt D, float e, pt E, float f, pt F, float t) {
  pt G = L(B,C,(t-a)/(d-a)), H = L(C,D,(t-b)/(e-b)), I = L(D,E,(t-c)/(f-c)), 
                 J = L(G,H,(t-b)/(d-b)),         K = L(H,I,(t-c)/(e-c)),
                            L = L(J,K,(t-c)/(d-c));
   return L;
  }
  
pt S(pt A, pt B, pt C, pt D, pt E, pt F, float t) {
  float a=0, b=d(A,B), c=b+d(B,C), d=c+d(C,D), e=d+d(D,E), f=e+d(E,F); 
  return S(a,A,b,B,c,C,d,D,e,E,f,F,c+t*(d-c));}
...
//************************************************************************
// Parabola 
pt ParabolaQuarter(pt A, pt B, pt C) {return P(B).add(3./8,V(B,A)).add(1./8,V(C,B));}  
pt ParabolaLeft(pt A, pt B, pt C) {return P(B).add(1./4,V(B,A)).add(1./4,V(C,B)); }
void drawParabola(pt A, pt B, pt C) { beginShape(); for(float t=0; t<=1.01; t+=1./40) Parabola(A,B,C,t).v(); endShape();}

// Draw Bezier  curve
void drawBezier(pt A, pt B, pt C) {beginShape(); for(float t=0; t<=1.01; t+=1./40) B(A,B,C,t).v(); endShape(); }
void drawBezier(pt A, pt B, pt C, pt D) {beginShape(); for(float t=0; t<=1.01; t+=1./40) B(A,B,C,D,t).v(); endShape(); }
void drawBezier(pt A, pt B, pt C, pt D, pt E) {beginShape(); for(float t=0; t<=1.01; t+=1./40) B(A,B,C,D,E,t).v(); endShape(); }
void drawBezier(pt A, pt B, pt C, pt D, pt E, pt F) {beginShape(); for(float t=0; t<=1.01; t+=1./40) B(A,B,C,D,E,F,t).v(); endShape(); }

// Draw distance-based non-uniform interpolating curve
void drawCurveTrough(pt A, pt B, pt C) {beginShape(); for(float t=0; t<=1.001; t+=.1/40) v(D(A,B,C,t)); endShape();}
void drawCurveTrough(pt A, pt B, pt C, pt D) {beginShape(); for(float t=0; t<=1.001; t+=.1/40) v(D(A,B,C,D,t)); endShape();}
void drawCurveTrough(pt A, pt B, pt C, pt D, pt E) {beginShape(); for(float t=0; t<=1.001; t+=.1/40) v(D(A,B,C,D,E,t)); endShape();}
void drawCurveTrough(pt A, pt B, pt C, pt D, pt E, pt F) {beginShape(); for(float t=0; t<=1.001; t+=.1/40) v(D(A,B,C,D,E,F,t)); endShape();}

// Draw distance-based non-uniorm B-sline 
void drawNUBS(pt A, pt B, pt C, pt D, pt E, pt F) {beginShape(); for(float t=0; t<=1.01; t+=1./40) S(A,B,C,D,E,F,t).v(); endShape(); }
