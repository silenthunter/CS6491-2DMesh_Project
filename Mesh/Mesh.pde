...
class DelaunayMesh {
    pts P;
    ArrayList<pt> edges = new ArrayList<pt>(); // a pt is just 2 ints (x,y) which is now used as indices to points i.e. (src index, dest index)
...
//after cutting, spread the mesh  
void spreadMesh(int[] cutNodes){
  // unfix the cut pointsfs
...
void animate(){
  ...
  for(int i = 0; i < G.length; i++){
    if( true == G[i].fixed) continue; // skip the fixed points, can't be moved
    float dvx = 0, dvy = 0; // change in velocity
    float dx = 0, dy = 0;  // change in displacement
    
    // find all edges incident on G[i] since each edge contributes a vector
    for(int j = 0; j < edges.size(); j++) {
      
      pt currEdge = edges.get(j), src, dest;
      int srcIndex, destIndex;
      srcIndex = (int)currEdge.x;
      destIndex = (int)currEdge.y;
      
      //edge found
      if(srcIndex == i || destIndex == i){
        int otherIndex = ((srcIndex == i) ? destIndex : srcIndex);
        src = points[srcIndex];
        dest = points[destIndex];
        float springLength = d(src, dest);
        float acc = (springLength - REST_LENGTH) * SPRING_CONSTANT / DEFAULT_MASS; // hooke's law
        ...
        if(acc < 0){
          // push i away
          if(G[i].x > G[otherIndex].x){
            dvx += (Math.abs(ratio) * Math.abs(src.x-dest.x)) * DT;
       //     dvx -= (Math.abs(dampingratio) * Math.abs(src.x-dest.x)) * DT;
          }else{
            dvx -= (Math.abs(ratio) * Math.abs(src.x-dest.x)) * DT;
       //     dvx += (Math.abs(dampingratio) * Math.abs(src.x-dest.x)) * DT;
          }
          
          if(G[i].y > G[otherIndex].y){
            dvy += (Math.abs(ratio) * Math.abs(src.y-dest.y)) * DT;
  ...
  //update position for all unfixed points
  for(int i = 0; i < G.length; i++){
    if( true == G[i].fixed) continue; // skip the fixed points, can't be moved
//    if(G[i].velocityx < EPSILON) G[i].velocityx = 0;
  //  if(G[i].velocityy < EPSILON) G[i].velocityy = 0;
    G[i].x += G[i].velocityx * DT;
    G[i].y += G[i].velocityy * DT;
  }
}
