...
class pts {
 ...
 pt[] G = new pt [maxnv];                 // geometry table (vertices)
  ...
  pts copyFrom(pts Q) {nv=0; for (int v=0; v<Q.nv; v++) addPt(Q.G[v]); return this;}
  ...
  pts drawDisplaced(int i, float r) {
    if(i>0&&i<nv-1) show(P(G[i],4,W(V(G[i],G[i-1]),V(G[i],G[i+1]))),r); 
    else show(G[i],r); 
    return this;}

 // mid-edge points
  pts drawMidPointsX() {for (int p=nv-1, v=0; v<nv; p=v, v++) P(G[p],G[v]).show(2); return this;}
  pts drawMidPoints() {for (int v=0; v<nv-1; v++) MidEdge(v).show(2); if(loop) MidEdge(nv-1).show(2); return this;}
  pt MidEdge(int v) {if(v==nv-1) return P(G[v],G[0]); else return P(G[v],G[v+1]);}
  pts pickClosestMidEdge(pt M) {
   pv=0; for (int i=1; i<nv-1; i++) if (d(M,MidEdge(i))<d(M,MidEdge(pv))) pv=i; 
   if(loop) {if (d(M,MidEdge(nv-1))<d(M,MidEdge(pv))) pv=nv-1;}
   return this;
   }
  ...
  // ******** smoothing
  pts tuck(float s) {
    pt[] S = new pt [nv]; // temporary array
    // copy of each intermediate point moved by s towards the average of its neighbors
    for (int v=1; v<nv-1; v++) S[v]=L(G[v],P(G[v-1],G[v+1]),s);  // S = G + s((P+N)/2-G)
